...
import { fetchPremiumProperties } from "@/lib/supabase/homepage";
...
export function PremiumProperties() {
    ...
    useEffect(() => {
        ...
        fetchPremiumProperties(6, selectedCity || undefined)
            .then((data) => {
                if (!cancelled) setProperties(data);
            })
            .catch((err) => console.error("[Premium] fetch error:", err))
            .finally(() => {
            ...
            <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm md:text-base text-slate-500 mt-1">
                        Handpicked premium properties{selectedCity ? ` in ${selectedCity}` : ""}
                    </p>
                </div>
                <Link
                    href="/search"
                    className="text-sm font-medium text-primary hover:text-primary-hover transition-colors"
                    aria-label="View all premium properties"
                >
                    View All
                </Link>
            </div>
