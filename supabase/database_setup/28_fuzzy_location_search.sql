-- ==========================================
-- 28_FUZZY_LOCATION_SEARCH.SQL
-- Updates search_properties to match across
-- locality, address, AND city columns for
-- fuzzy text-based location search.
--
-- SAFETY:
--   - Wrapped in a transaction (BEGIN/COMMIT)
--   - If the CREATE fails, the DROP is rolled back
--     and your old function stays intact
--   - Does NOT touch the properties table or any data
--   - Only replaces function logic (adds locality/state
--     to return + fuzzy WHERE clause)
-- ==========================================

BEGIN;

-- Must DROP first because the return type changed
-- (adding locality + state columns that didn't exist before)
...
BEGIN
  ...
  WHERE p.status = 'approved'
    -- Fuzzy location matching across city, locality, AND address
    AND (p_city IS NULL 
